...
**Transporte — nav "Tarifario" nuevo (CEDOL sin acompañante).** Se agregó al dashboard de Transporte (`Logistica4`) un nav "Tarifario" con: (1) tabla base de tarifas may-26 (4 vehículos × 3 columnas), (2) panel editable de evolución del índice CEDOL "Distribución Urbana sin acompañante" — columna "mi dato" verificada (fuente cedol.org.ar: may-26=431.414,53, jun-26=446.801,36 → +3,57%) + columna "tu dato" editable con persistencia localStorage, (3) tarifas ajustadas por el acumulado CEDOL con cortes cuatrimestrales resaltados (base may-26, aumentos cada 4 meses). El índice CEDOL sin acompañante NO se publica limpio todos los meses en prensa; los valores mensuales se cargan a mano desde el informe oficial. IMPORTANTE: `Logistica4` se re-sincroniza a diario desde el repo fuente `braianch94-droid/Logistica4` (sync.yml, 5 AM), así que el cambio se pusheó AL REPO FUENTE (commit f816c85 en main) además del portal, para que no lo pise el sync. Resuelto y permanente.

**Portal — ocultados del nav: Inventario Cíclico ARG + todo Zecat Chile.** A pedido, con reglas CSS reversibles en `index.html`: `.section:has(.nav-item[data-module="ciclico"])` (cíclico ARG) y `.country-group:has(.country-hdr[title="Zecat Chile"])` (grupo Chile completo: su productividad, cíclico, etc.). Los dashboards siguen existiendo; para revertir, borrar esas reglas.

**Descarga Infor rota (Compass/Monaco cambió) — SIGUE PENDIENTE.** Desde ~27/07 la descarga automática falla: Infor cambió la pantalla del editor de queries y `window.monaco` ya no aparece en ningún frame (`setValue -> no-monaco`). Se intentaron fixes a ciegas (más espera, buscar Monaco en todos los frames) sin éxito. Para arreglarlo hay que inspeccionar la UI de Compass en vivo (requiere login del usuario en Infor — no hay sesión activa) o exportar las olas a mano. Dashboard de productividad quedó al 24/07.
